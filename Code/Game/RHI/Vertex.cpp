...
//---------------------------------------------------------------------------------------------------------------------------
size_t CreateVertex(byte* buffer, size_t bufferSize, const Vector3& pos, const RGBA& col, const Vector2& tex, const Vector3& tangent, const Vector3& bitangent, const Vector3& normal) {
	memcpy_s(buffer, bufferSize, (void*)&pos, sizeof(Vector3));
	buffer += sizeof(Vector3);
	memcpy_s(buffer, bufferSize, (void*)&col, sizeof(RGBA));
	buffer += sizeof(RGBA);
	...
	buffer += sizeof(Vector3);
	memcpy_s(buffer, bufferSize, (void*)&normal, sizeof(Vector3));
	return sizeof(Vector3) + sizeof(RGBA) + sizeof(Vector2) + sizeof(Vector3) + sizeof(Vector3) + sizeof(Vector3);
}
...
uint16_t GetVertexByteSize(eVertexType vertType) {
	...
	case VERTEX_TYPE_PCTTBN: {
		return sizeof(Vector3) + sizeof(RGBA) + sizeof(Vector2) + 3 * sizeof(Vector3);
	}
	case VERTEX_TYPE_PCTTBNBN: {
		return sizeof(Vector3) + sizeof(RGBA) + sizeof(Vector2) + 3 * sizeof(Vector3) + sizeof(Vector4) + sizeof(IntVector4);
	}
...
void SetInputLayout(eVertexType vertType, D3D11_INPUT_ELEMENT_DESC*& layout, UINT& numElements) {
	...
	case VERTEX_TYPE_PCTN: {
		...
		layout[1] = { "COLOR",		0, DXGI_FORMAT_R32G32B32A32_FLOAT,	0, 12,	D3D11_INPUT_PER_VERTEX_DATA, 0 };
		layout[2] = { "TEXCOORD",	0, DXGI_FORMAT_R32G32_FLOAT,		0, 28,	D3D11_INPUT_PER_VERTEX_DATA, 0 };
		layout[3] = { "NORMAL",		0, DXGI_FORMAT_R32G32B32_FLOAT,		0, 36,	D3D11_INPUT_PER_VERTEX_DATA, 0 };
		numElements = 4;
		break;
	}
	case VERTEX_TYPE_PCTTBN: {
		...
		layout[1] = { "COLOR",		0, DXGI_FORMAT_R32G32B32A32_FLOAT,	0, 12,	D3D11_INPUT_PER_VERTEX_DATA, 0 };
		layout[2] = { "TEXCOORD",	0, DXGI_FORMAT_R32G32_FLOAT,		0, 28,	D3D11_INPUT_PER_VERTEX_DATA, 0 };
		layout[3] = { "TANGENT",	0, DXGI_FORMAT_R32G32B32_FLOAT,		0, 36,	D3D11_INPUT_PER_VERTEX_DATA, 0 };
		layout[4] = { "BITANGENT",	0, DXGI_FORMAT_R32G32B32_FLOAT,		0, 48,	D3D11_INPUT_PER_VERTEX_DATA, 0 };
		layout[5] = { "NORMAL",		0, DXGI_FORMAT_R32G32B32_FLOAT,		0, 60,	D3D11_INPUT_PER_VERTEX_DATA, 0 };
		numElements = 6;
		break;
	}
	case VERTEX_TYPE_PCTTBNBN: {
